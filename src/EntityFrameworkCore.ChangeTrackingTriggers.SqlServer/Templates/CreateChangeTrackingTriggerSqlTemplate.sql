...
BEGIN
	...
	SET @ChangeSource = CAST(SESSION_CONTEXT(N'{{change_tracking_context_change_source_name}}') AS {{string.upcase change_source_column.type}})
	IF @ChangeSource IS NULL
	BEGIN
		;THROW 130101, '{{change_tracking_context_change_source_name}} must be set in session context for change tracking. Transaction was not commited.', 1
	END
	{{~ end ~}}
	{{~ if changed_by_column != null ~}}

	SET @ChangedBy = CAST(SESSION_CONTEXT(N'{{change_tracking_context_changed_by_name}}') AS {{string.upcase changed_by_column.type}})
	IF @ChangedBy IS NULL
	BEGIN
		;THROW 130101, '{{change_tracking_context_changed_by_name}} must be set in session context for change tracking. Transaction was not commited.', 1
	END
	{{~ end ~}}
