...
namespace EFCore.ChangeTriggers.Interceptors
{
    internal abstract class BaseChangedByDbConnectionInterceptor<TChangedBy> : DbConnectionInterceptor
    {
        ...
        public override async Task ConnectionOpenedAsync(DbConnection connection, ConnectionEndEventData eventData, CancellationToken cancellationToken = new())
        {
            ...
            await SetChangedByChangeContextAsync(connection, changedByProviderValue, cancellationToken);
        }

        protected abstract void SetChangedByChangeContext(DbConnection connection, object? changedByProviderValue);

        protected abstract Task SetChangedByChangeContextAsync(DbConnection connection, object? changedByProviderValue, CancellationToken cancellationToken);
    }
}
